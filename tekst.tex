...
Bij het testen gebruiken we alleen orgels waarvan we de prijs kennen. Het programma maakt per orgel prijsschatting en schrijft deze weg naar het geheugen. Als alle orgels doorlopen zijn, gaan we de error-waardes berekenen. Bij de formule-tabel, Tabel \ref{tab:error_formulas}, is voor het \(i\)-de orgel \(s_i\) de geschatte prijs, \(p_i\) de \(i\)-de echte prijs en \(N\) het totaal aantal orgels.
...
\begin{table}[ht]
    ...
    \begin{tabular}{ll}
        ...
        Relative error: & \(\displaystyle\frac{1}{N}\sum_{i=1}^{N}\frac{|s_i - p_i|}{p_i}\)\\
